...
set_up_model <- function(config,
  ...
  FLARErLER:::update_var(length(non_temp_names), "num_wq_vars", ens_working_directory, "glm3.nml") #GLM SPECIFIC

  if(length(non_temp_names) > 1) {
    FLARErLER:::update_var(non_temp_names, "wq_names", ens_working_directory, "glm3.nml")
  }

  if(!is.null(ncol(inflow_file_names))) {
    FLARErLER:::update_var(ncol(inflow_file_names), "num_inflows", ens_working_directory, "glm3.nml")
    FLARErLER:::update_var(ncol(outflow_file_names), "num_outlet", ens_working_directory, "glm3.nml")
    inflow_var_names <- c("FLOW","TEMP","SALT", non_temp_names)
    FLARErLER:::update_var(inflow_var_names, "inflow_vars", ens_working_directory, "glm3.nml")
    FLARErLER:::update_var(length(inflow_var_names), "inflow_varnum", ens_working_directory, "glm3.nml")
  }
  ...
  FLARErLER:::update_var(length(config$modeled_depths), "num_depths", ens_working_directory, "glm3.nml") #GLM SPECIFIC
